...
class Program
{
    ...
    static void GetStockData(string ticker)
    {
        ...
        string url =
            $"https://query1.finance.yahoo.com/v7/finance/download/{ticker}?period1={startTimestamp}&period2={endTimestamp}&interval=1d&events=history&includeAdjustedClose=true";
        ...
        using (WebClient client = new WebClient())
        {
            try
            {
                ...
                for (int i = 1; i < lines.Length; i++)
                {
                    ...
                    prices[i - 1] = double.Parse(columns[5]);    //получение цен
                }

                double averagePrice = prices.Length > 0 ? prices.Average() : 0;

                lock (locker)  //безопасность
                {
